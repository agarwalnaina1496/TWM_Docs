...
`destination.type` — `single` or `circuit`. Determines how Trip Planner reads the destination.

`destination.name` — populated for single destinations. Null for circuits.

`destination.stops` — populated for circuits. Array of stop objects with name, nights, and role. Empty for single destinations.

`destination.confirmed_via` — `matcher` or `user_provided`. Useful for analytics and future personalisation.

`destination.shortlist_considered` — the alternatives Meridian surfaced before confirmation. Context for Trip Planner.

`matcher_inputs` — accumulates across the Scout conversation. Passed back to Scout on each call so the conversation can resume without restarting. Null fields are omitted when sending to Meridian.
