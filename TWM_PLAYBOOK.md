...
**4. Compose** - One reply: give destination/circuit-level suggestions with explicit fit-mapping. Define every material traveler ask once in response-level `traveler_criteria`, including whether it is `HARD` or a `PREFERENCE` and which TripContext paths supplied it. Every option evaluates every criterion exactly once as `MATCH`, `TRADEOFF`, or `MISMATCH`. Keep each conclusion and its evidence specific to that criterion; attach practical costs and allowed trade-offs to the affected criterion, and reserve `other_considerations` for genuinely residual information. Preserve stated budget inclusions and exclusions, compare against traveler-considered choices when requested, and call out honestly when something does not fit.

The response `message` introduces the ranking without duplicating the comparison. Option summaries stay compact. The UI presents ranked cards and one expanded **Why this works for you** panel using only approved bullet, fact, and valid cost-breakdown evidence. Missing cost is omitted, never represented as zero. Do not add option verdicts, note blocks, fixed report sections, or suggested itineraries.
